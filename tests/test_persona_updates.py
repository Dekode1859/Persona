...
from spiritus import ReleaseCandidate, StagedUpdate, UpdateArtifact, UpdateCheck, UpdateStatus
...
class _FakeClient:
    ...
    def stage_update(self, result: UpdateCheck, destination: Path) -> StagedUpdate:
        ...
        return StagedUpdate(self.staged.artifact, target, self.staged.bytes, self.staged.sha256)
...
class PersonaUpdateTests(unittest.TestCase):
    ...
    def test_staged_update_is_restored_after_startup(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            client = _FakeClient(StagedUpdate(artifact, source, len(payload), digest))
